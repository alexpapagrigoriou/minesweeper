...
static void render_exit(void) {
    ...
    mvaddstr(game_exit_start_row, game_exit_start_col, "  X  ");
...
static void render_board(Board* board) {
    render_filled_box(game_board_start_row + 1, game_board_start_col + 1, BOARD_ROWS - 2, BOARD_COLS - 2, CP_CELL);

    render_grid(game_board_start_row, game_board_start_col, BOARD_ROWS, BOARD_COLS, CELL_WALL_SIZE, CP_GRID);
    ...
    for (int row = 0; row < BOARD_SIZE; row++) {
        for (int col = 0; col < BOARD_SIZE; col++) {
            ...
            if (board_is_flagged(board, index)) {
                int cell_row = game_board_start_row + 1 + row * CELL_WALL_SIZE;
                int cell_col = game_board_start_col + 1 + col * CELL_WALL_SIZE;
...
static void render_won_lost(bool won) {
    if (won) {
        render_text_centered_col(game_board_start_row - 1, " Victory! ", CP_WON, A_BOLD);
    } else {
        render_text_centered_col(game_board_start_row - 1, " Game Over! ", CP_LOST, A_BOLD);
    }
}

static void render_play_again(void) {
    render_filled_box(game_play_again_start_row, game_play_again_start_col, PLAY_AGAIN_ROWS, PLAY_AGAIN_COLS, CP_PLAY_AGAIN);
    render_text_centered_col(game_play_again_start_row + (PLAY_AGAIN_ROWS / 2), "Play Again", CP_PLAY_AGAIN, A_BOLD);
}
